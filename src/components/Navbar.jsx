  ...
  return (
    <div className={`sticky top-0 z-[1000] w-full transition-all duration-200 ${hasScrolled ? 'bg-black' : 'bg-transparent'}`}>
      <div className='flex justify-between items-center px-4 py-2 text-white'>
        ...
        <nav className={`lg:flex hidden space-x-4 text-lg`}>
      ...
      {/* Mobile Menu */}
      <div className={`fixed inset-0 bg-black bg-opacity-90 w-full h-full flex flex-col items-center justify-between transition-transform transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} lg:hidden`}>
